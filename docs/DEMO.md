...
| 04:15 | Audit log | Open any `audit_logs/cognitive-*.jsonl` in a tail. | Point at the score inputs, the weights snapshot, the model source, and the `priors_active` flag. Every cognitive evaluation, every prescription, and every what-if replay writes its audit row before the matching Kafka emit. |
| 04:30 | Live PPG (optional) | Open `http://<laptop LAN IP>:3000/sensor` on a phone on the same WiFi. Tap Start, fingertip over the rear camera. | Within fifteen seconds the live BPM appears on the phone and the stress ring on the dashboard reacts. The biometric layer is no longer purely synthetic. Events land on `biometrics-enriched` tagged `source: "ppg-camera"`. |
| 04:45 | Close | Land the differentiator. | Other systems ask what is happening to the car. NeuroPit infers the human nervous system operating the car, prescribes the next pit-wall action, calibrates persona thresholds per driver against real telemetry, ingests live biometrics from a phone, and lets the strategist defend the call after the fact. |
